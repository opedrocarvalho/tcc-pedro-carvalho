...
O processo de \acs{etl} consolidou-se como o componente fundamental na \acs{ed}, responsável por extrair informações de diversas fontes, transformá-las conforme necessidades analíticas e carregá-las em ambientes como \acs{dw} ou \acs{dl}. O DW é uma estrutura de banco de dados desnormalizado e centralizado, projetada para fornecer suporte à tomada de decisão estratégica por meio de análise histórica, amplamente utilizada em operações de \ac{olap} e \textit{data mining} (\cite[]{septian2019}).

Em contrapartida, o DL representa uma abordagem eficiente e escalável para o armazenamento de grandes volumes de dados brutos em diversos formatos, permitindo que o processamento ocorra sob demanda. Com suporte a ferramentas \acs{open_source}, como o Hadoop File System, os DLs oferecem vantagens como baixo custo, tolerância a falhas e flexibilidade de esquema (\textit{schema on read}).

Essa evolução arquitetural é um reflexo direto dos desafios impostos pelo \textit{Big Data}. Segundo \cite[]{nwokeji2021}, o ETL evoluiu para lidar com os cinco principais desafios do Big Data, conhecidos como os 5Vs: volume, variedade, velocidade, veracidade e valor. No entanto, processos tradicionais de ETL, tipicamente associados ao contexto de \ac{bi}, frequentemente demonstram limitações em ambientes modernos, incluindo custo elevado e ingestão limitada em tempo real (\cite[]{nwokeji2021}).

Para superar essas restrições, houve uma evolução significativa nos modelos. \cite[]{boulahia2024} categorizam o ETL em três grandes vertentes: ETL para BI (tradicional, focado em dados estruturados), ETL para \textit{Big Data} (utilizando tecnologias distribuídas como Hadoop e Spark para escalabilidade) e ETL Semântico (que utiliza ontologias para integração de dados heterogêneos).

Em paralelo, a própria ordem do processamento foi redefinida. Surgiu a abordagem ELT, na qual os dados brutos são primeiramente carregados no \acs{dl} ou \acs{dw} e a transformação ocorre posteriormente, explorando o poder de processamento do próprio destino. Essa flexibilidade é crucial para o desenvolvimento de \textit{pipelines} híbridos e escaláveis.

\subsubsection{Transição para ELT e o Papel do \textit{Data Wrangling}}
\label{ssec:transicao_elt}

Um desafio central que afeta a escolha é a \textbf{complexidade} do \textit{Data Wrangling} (obter, organizar, entender, extrair e formatar dados) . A complexidade e a escala do \textit{Big Data} impulsionaram a transição do modelo tradicional ETL para o \ac{elt} . No modelo ELT, a transformação ocorre após o carregamento dos dados brutos no \acs{dl}, aproveitando a capacidade de processamento distribuído da plataforma .

Apesar do desafio de \textit{setup} e manutenção, o modelo \acs{open_source}, exemplificado pelo Apache Airflow, é a opção preferida por sua escalabilidade, flexibilidade e a capacidade de ser um "contêiner aberto" para diversas soluções de \textit{data wrangling} .
...
\end{itemize}
Desta forma, ambientes acadêmicos e empresas com foco em inovação e adaptabilidade podem beneficiar-se das soluções \acs{open_source}, enquanto grandes corporações com alta criticidade de dados e requisitos regulatórios podem ainda optar por ferramentas proprietárias (\cite[]{boulahia2024}, \cite[]{septian2019}).
