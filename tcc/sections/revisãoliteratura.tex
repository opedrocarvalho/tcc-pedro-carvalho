...
A escassez de artigos diretamente identificados pela \textit{string} de busca inicial, um desafio comum em revisões bibliográficas de escopo restrito ou emergente, motivou a adoção complementar da técnica de \textit{Forward Snowballing} \cite[]{felizardo2016using}. Essa abordagem consiste em identificar trabalhos que citam os artigos já selecionados, permitindo a descoberta de publicações relevantes que não foram capturadas pela \textit{string} de busca original. O processo foi executado sobre os seis artigos selecionados na revisão, rastreando citações posteriores por meio do Google Scholar, e resultou na identificação de \textbf{três trabalhos adicionais} de alta relevância para o tema.

O primeiro foi o trabalho de \cite{figueira2025arquitetura}, intitulado \textit{Arquitetura para Gestão Eficiente de Dados Turísticos}, que apresenta ferramentas como PostgreSQL, Docker e Apache Superset aplicadas em um \textit{pipeline} de dados de turismo, domínio análogo ao deste trabalho. Tecnologias que, por sua relevância e aderência ao problema investigado, foram incorporadas à presente proposta. O segundo foi \cite{jin2025elt}, \textit{ELT-Bench: An End-to-End Benchmark for Evaluating AI Agents on ELT Pipelines}, que propõe o primeiro \textit{benchmark} \textit{end-to-end} para pipelines ELT, evidenciando a crescente relevância do paradigma ELT e o papel central de ferramentas como o dbt nesse ecossistema. O terceiro foi \cite{gargouri2026multi}, \textit{A Multi-Layer Testing Framework for Automated Data Quality Assurance in Cloud-Native ELT Pipelines}, que propõe uma estrutura de testes multicamada para pipelines ELT em ambientes \textit{cloud-native}, orquestrando verificações de consistência locais utilizando o DuckDB e o dbt antes da migração para \textit{data warehouses} em nuvem, citando diretamente o trabalho de \cite{mbata2024survey} sobre ferramentas de pipeline. Todos os três artigos encontrados via \textit{snowballing}, em especial \cite{gargouri2026multi} e \cite{jin2025elt}, introduzem diretamente ferramentas-chave utilizadas na implementação: o \textbf{DuckDB} como banco analítico embutido para a camada \textit{raw} e o \textbf{dbt} como motor de transformação declarativa na abordagem ELT, ambos validados empiricamente nesses estudos antes de serem incorporados à arquitetura deste trabalho.
