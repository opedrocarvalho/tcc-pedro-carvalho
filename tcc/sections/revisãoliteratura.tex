...
A escassez de artigos diretamente identificados pela \textit{string} de busca inicial, um desafio comum em revisões bibliográficas de escopo restrito ou emergente, motivou a adoção complementar da técnica de \textit{Forward Snowballing} \cite[]{felizardo2016using}. Essa abordagem consiste em identificar trabalhos que citam os artigos já selecionados, permitindo a descoberta de publicações relevantes que não foram capturadas pela \textit{string} de busca original. O processo foi executado sobre os seis artigos selecionados na revisão, rastreando citações posteriores por meio do Google Scholar, e resultou na identificação de \textbf{cinco trabalhos adicionais} de alta relevância para o tema.

O primeiro foi o trabalho de \cite{figueira2025arquitetura}, intitulado \textit{Arquitetura para Gestão Eficiente de Dados Turísticos}, que apresenta ferramentas como PostgreSQL, Docker e Apache Superset aplicadas em um \textit{pipeline} de dados de turismo — domínio análogo ao deste trabalho. Tecnologias que, por sua relevância e aderência ao problema investigado, foram incorporadas à presente proposta. O segundo foi \cite{mbata2024survey}, \textit{A Survey of Pipeline Tools for Data Engineering}, um levantamento abrangente das categorias ETL/ELT, ingestão, orquestração e aprendizado de máquina, que classifica e compara dezenas de ferramentas de código aberto e é citado múltiplas vezes ao longo desta revisão. O terceiro foi \cite{jin2025elt}, \textit{ELT-Bench: An End-to-End Benchmark for Evaluating AI Agents on ELT Pipelines}, que propõe o primeiro \textit{benchmark} \textit{end-to-end} para pipelines ELT, evidenciando a crescente relevância do paradigma ELT e o papel central de ferramentas como o dbt nesse ecossistema. O quarto foi \cite{gargouri2026multi}, \textit{A Multi-Layer Testing Framework for Automated Data Quality Assurance in Cloud-Native ELT Pipelines}, que propõe uma estrutura de testes multicamada para pipelines ELT em ambientes \textit{cloud-native}, orquestrando verificações de consistência locais utilizando o DuckDB e o dbt antes da migração para \textit{data warehouses} em nuvem, citando diretamente o trabalho de \cite{mbata2024survey} sobre ferramentas de pipeline. O quinto trabalho, \cite{felizardo2016using}, \textit{Using Forward Snowballing to Update Systematic Reviews in Software Engineering}, fundamenta metodologicamente a própria técnica de busca complementar empregada nesta revisão, demonstrando que o \textit{Forward Snowballing} reduz em mais de cinco vezes o número de estudos a analisar em relação à busca por string, com precisão superior (7,55\% contra 1,42\%), e aprofundando os critérios de qualidade e evolução das arquiteturas de dados, o que reforçou a adequação das escolhas tecnológicas adotadas neste projeto. Todos os cinco artigos encontrados via \textit{snowballing}, em especial \cite{gargouri2026multi} e \cite{jin2025elt}, introduzem diretamente ferramentas-chave utilizadas na implementação: o \textbf{DuckDB} como banco analítico embutido para a camada \textit{raw} e o \textbf{dbt} como motor de transformação declarativa na abordagem ELT, ambos validados empiricamente nesses estudos antes de serem incorporados à arquitetura deste trabalho.

Esta abordagem pragmática, porém metodologicamente fundamentada, permitiu a inclusão de informações cruciais e altamente relevantes de um conjunto mais amplo de fontes, garantindo a robustez necessária para responder de forma completa e fundamentada às questões de pesquisa propostas.

\begin{subsection}{Artigos Selecionados para a Revisão}
    
    A execução dessa estratégia de busca retornou inicialmente um total de \textbf{315 publicações}. Para organizar o processo de triagem, foi utilizada a ferramenta \textbf{Parsifal}, amplamente empregada em revisões sistemáticas da literatura. Após a leitura dos resumos, aplicação de critérios de inclusão e exclusão e a verificação de alinhamento com as \textit{Research Questions}, apenas \textbf{6 artigos foram selecionados} para compor a presente revisão. Adicionalmente, a aplicação da técnica de \textit{Forward Snowballing} \cite[]{felizardo2016using} sobre esses 6 trabalhos resultou na identificação de \textbf{3 publicações complementares}, incorporadas conforme descrito na seção anterior. Esses trabalhos constituem a base para a análise de avanços recentes, comparação de ferramentas e identificação de tendências emergentes no campo dos pipelines de dados. A tabela \ref{tab:artigos_selecionados} apresenta um resumo dos artigos selecionados pela busca inicial.

\input{tables/artigos.tex}

\end{subsection}

\section{Respostas às Questões de Pesquisa}

A análise da literatura selecionada, complementada pelo \textit{Forward Snowballing} \cite[]{felizardo2016using}, permite responder às questões de pesquisa propostas, traçando um panorama atualizado sobre as ferramentas open source, suas vantagens, desvantagens e o contexto de aplicação em pipelines de dados.
